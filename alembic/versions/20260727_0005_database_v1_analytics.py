...
def upgrade() -> None:
    ...
    op.execute(
        ...
        CREATE TABLE analytics.dim_companies (
            company_key BIGINT GENERATED BY DEFAULT AS IDENTITY,
    ...
    op.execute(
        ...
        CREATE TABLE analytics.dim_skills (
            skill_key BIGINT GENERATED BY DEFAULT AS IDENTITY,
    ...
    op.execute(
        ...
        BEGIN
            IF TG_TABLE_NAME = 'dim_sources'
               AND (((to_jsonb(NEW)->>'source_key')::bigint IS DISTINCT FROM
                     (to_jsonb(OLD)->>'source_key')::bigint)
                    OR ((to_jsonb(NEW)->>'source_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'source_id')::uuid)) THEN
                RAISE EXCEPTION 'analytics.dim_sources identity is immutable'
                    USING ERRCODE = '23514';
            ELSIF TG_TABLE_NAME = 'dim_companies'
               AND (((to_jsonb(NEW)->>'company_key')::bigint IS DISTINCT FROM
                     (to_jsonb(OLD)->>'company_key')::bigint)
                    OR ((to_jsonb(NEW)->>'company_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'company_id')::uuid)) THEN
                RAISE EXCEPTION 'analytics.dim_companies identity is immutable'
                    USING ERRCODE = '23514';
            ELSIF TG_TABLE_NAME = 'dim_locations'
               AND (((to_jsonb(NEW)->>'location_key')::bigint IS DISTINCT FROM
                     (to_jsonb(OLD)->>'location_key')::bigint)
                    OR ((to_jsonb(NEW)->>'location_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'location_id')::uuid)) THEN
                RAISE EXCEPTION 'analytics.dim_locations identity is immutable'
                    USING ERRCODE = '23514';
            ELSIF TG_TABLE_NAME = 'dim_occupations'
               AND (((to_jsonb(NEW)->>'occupation_key')::bigint IS DISTINCT FROM
                     (to_jsonb(OLD)->>'occupation_key')::bigint)
                    OR ((to_jsonb(NEW)->>'occupation_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'occupation_id')::uuid)
                    OR ((to_jsonb(NEW)->>'taxonomy_version_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'taxonomy_version_id')::uuid)) THEN
                RAISE EXCEPTION 'analytics.dim_occupations identity is immutable'
                    USING ERRCODE = '23514';
            ELSIF TG_TABLE_NAME = 'dim_skills'
               AND (((to_jsonb(NEW)->>'skill_key')::bigint IS DISTINCT FROM
                     (to_jsonb(OLD)->>'skill_key')::bigint)
                    OR ((to_jsonb(NEW)->>'skill_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'skill_id')::uuid)
                    OR ((to_jsonb(NEW)->>'taxonomy_version_id')::uuid IS DISTINCT FROM
                        (to_jsonb(OLD)->>'taxonomy_version_id')::uuid)) THEN
